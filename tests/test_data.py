...
if __name__ == "__main__":
    ...
    eurusd_data = TestData.generate_ohlc_data(symbol='EURUSD', timeframe='H1')
    print(f"Wygenerowano {len(eurusd_data)} świec dla EURUSD H1")
    print(eurusd_data.head())
    
    # Dane z trendem wzrostowym
    uptrend_data = TestData.get_trending_market_data(symbol='EURUSD', trend='up')
    print(f"\nPierwsza cena zamknięcia: {uptrend_data.iloc[0]['close']}")
    print(f"Ostatnia cena zamknięcia: {uptrend_data.iloc[-1]['close']}")
    
    # Dane dla wielu ram czasowych
    multi_tf_data = TestData.get_multi_timeframe_data(symbol='EURUSD')
    for tf, data in multi_tf_data.items():
        print(f"\nRama czasowa {tf}: {len(data)} świec") 